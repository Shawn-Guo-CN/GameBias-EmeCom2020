...
def _populate_cl_params(arg_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    arg_parser.add_argument('--random_seed', type=int, default=1234,
    ...
    arg_parser.add_argument('--topo_path', type=str, default='log/topo_sim.txt',
                        help='The path to log file for topological similarity.')
    arg_parser.add_argument('--generalisation_path', type=str, default='log/generalisation_tmp.txt',
                        help='Path to log file for cross-task language learning.')
    arg_parser.add_argument('--training_log_path', type=str, default='log/training_log_tmp.txt',
                        help='Path to log file for training procedure.')
    # cuda setup
    arg_parser.add_argument('--no_cuda', default=False, help='disable cuda',
                        action='store_true')
    arg_parser.add_argument('--cuda_no', default=0, type=int, help='Number of GPU for cuda.')
    # dataset
    ...
    arg_parser.add_argument('--dataset', type=str, default='mnist',
                            help='Dataset')
    arg_parser.add_argument('--subsample', type=int, default=100,
                            help='Load every kth example')
    # Channel parameters
    ...
    arg_parser.add_argument('--beta', type=int, default=1,
                            help='beta-VAE value')
    arg_parser.add_argument('--z_dim', type=int, default=10,
                            help='z_dim')
...
def _get_params(arg_parser: argparse.ArgumentParser, params: List[str]) -> argparse.Namespace:
    ...
    if args.cuda:
        print(args.cuda_no)
        args.device = torch.device('cuda:'+str(args.cuda_no))
