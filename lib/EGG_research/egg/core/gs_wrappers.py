...
class RnnSenderGS(nn.Module):
    ...
    def forward(self, x):
        ...
        for step in range(self.max_len):
            ...
            gumb_msg = gumbel_softmax_sample(step_logits, self.temperature, self.training, self.straight_through)
            logits_seq.append(step_logits)

            prev_hidden = h_t
            e_t = self.embedding(gumb_msg)
            sequence.append(gumb_msg)

        sequence = torch.stack(sequence).permute(1, 0, 2)
        logits_seq = torch.stack(logits_seq).permute(1,0,2)

        return sequence, logits_seq
...
class SenderReceiverRnnGS(nn.Module):
    ...
    def forward(self, sender_input, labels, receiver_input=None):
        message, _ = self.sender(sender_input)
        receiver_output = self.receiver(message, receiver_input)
        ...
        logging_strategy = self.train_logging_strategy if self.training else self.test_logging_strategy
        interaction = logging_strategy.filtered_interaction(
            sender_input=labels,            # Here we change to calculate topsim
            receiver_input=receiver_input,
            labels=labels,
            receiver_output=receiver_output.detach(),
            message_length=torch.ones(message.size(0)),
