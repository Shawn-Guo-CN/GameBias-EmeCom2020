...
class CheckpointSaverBest(Callback):
    ...
    def __init__(
            ...
            prefix: str = '',
            acc_data_path: str = Union[str, pathlib.Path]
    ):
        self.checkpoint_path = pathlib.Path(checkpoint_path)
        self.checkpoint_freq = checkpoint_freq
        self.prefix = prefix
        ...
        self.topo = []
        self.new_data_acc = []
        self.acc_data_path = pathlib.Path(acc_data_path)

    def on_test_end(self, loss: float, logs: Dict[str, Any] = None):
        if self.checkpoint_freq > 0 and (self.epoch_counter % self.checkpoint_freq == 0):
            self.test_acc.append(logs['acc'])
            if 'topo' in logs:
                self.topo.append(logs['topo'])
            if 'acc_new_data' in logs:
                self.new_data_acc.append(logs['acc_new_data'])
            self.save_checkpoint_acc()
            if logs['acc'] > self.best_acc:
    ...
    def save_checkpoint_acc(self):
        self.acc_data_path.mkdir(exist_ok=True)
        d = {'train_acc': self.train_acc, 'test_acc': self.test_acc, 'topo': self.topo, 'new_data_acc': self.new_data_acc}
        filename = f'{self.prefix}_acc' if self.prefix else 'acc'
        path = self.acc_data_path / f'{filename}.npy'
        np.save(path, d)

    def get_checkpoint(self):
        return CheckpointBestAcc(epoch=self.epoch_counter,
                          model_state_dict=self.trainer.game.state_dict(),
                          optimizer_state_dict=self.trainer.optimizer.state_dict(),
