...
from egg.zoo.compo_vs_gen_extended.archs import Sender, Receiver, \
    Freezer, PlusOneWrapper, NonLinearReceiver
from egg.zoo.compo_vs_gen_extended.intervention import Metrics, Evaluator
from egg.zoo.data_loaders.data_loaders import get_dsprites_dataloader
...
def get_params(params):
    ...
    parser.add_argument('--n_attributes', type=int, default=6, help='')
    parser.add_argument('--n_values', type=int, default=40, help='')
...
class DiffLoss(torch.nn.Module):
    ...
    def forward(self, sender_input, _message, _receiver_input, receiver_output, _labels):
        ...
        sender_input = sender_input.view(
            batch_size, self.n_attributes, 1)
...
def main(params):
    ...
    train_loader, validation_loader = get_dsprites_dataloader(path_to_data='egg/zoo/data_loaders/data/dsprites.npz',
                                                        batch_size=opts.batch_size, subsample=opts.subsample,
                                                        image=False)
    ...
    metrics_evaluator = Metrics(latent_values, opts.device, opts.n_attributes,
